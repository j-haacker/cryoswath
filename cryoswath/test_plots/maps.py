...
def coverage(l3_data_or_filepath: xr.Dataset|str,
    ...
    # print(l3_data)
    o2_code = misc.find_region_id(shapely.box(*l3_data._median.isel(time=0).rio.reproject(4326).rio.bounds()), scope="o2")
    ...
    sns.ecdfplot(ds._iqr.where(ds._count>=3).fillna(999).rio.clip([glacier_complexes.unary_union]).to_numpy().flatten())
    plt.xlim([0, 50])
    ...
    ds._iqr.where(ds._count>=3).median("time").fillna(999).rio.clip([glacier_complexes.unary_union]).plot(vmin=5, vmax=30, cbar_kwargs={"label": "Inter-quartile range, m"})
    glacier_complexes.boundary.plot(ax=plt.gca())
    plt.title("")
    plt.gca().axis("off")
    return plt.gcf()
