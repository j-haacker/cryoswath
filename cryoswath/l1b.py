...
def read_esa_l1b(
    ...
    poly3fit_params = np.polyfit(np.arange(len(ds.time_20_ku)-1), 
                                 WGS84_ellpsoid.inv(lats1=ds.lat_20_ku[:-1], lons1=ds.lon_20_ku[:-1],
                                                 lats2=ds.lat_20_ku[1:], lons2=ds.lon_20_ku[1:])[0],
                                 3)
    ds = ds.assign(azimuth=("time_20_ku", np.poly1d(poly3fit_params)(np.arange(len(ds.time_20_ku)-.5))%360))
    ds["power_waveform_20_ku"] = ds.pwr_waveform_20_ku \
                                 * ds.echo_scale_factor_20_ku \
                                 * 2**ds.echo_scale_pwr_20_ku
    if not use_original_noise_estimates:
        ...
        if len(ds.time_20_ku) > 2*(tracking_cycles*20):
            noise = xr.apply_ufunc(noise_val, ds.power_waveform_20_ku.isel(ns_20_ku=slice(int(len(ds.ns_20_ku)/4))), input_core_dims=[["ns_20_ku"]], output_core_dims=[[]], vectorize=True)
    ...
    if waveform_selection is not None:
        ...
        if (isinstance(waveform_selection, slice) and isinstance(waveform_selection.start, numbers.Integral)) \
            ...
            ds = ds.sel(time_20_ku=waveform_selection, method="nearest",
                            tolerance=np.timedelta64(25, "ms"))
    if mask_coherence_gt1:
        ds["coherence_waveform_20_ku"] = ds.coherence_waveform_20_ku.where(ds.coherence_waveform_20_ku <= 1)
    ...
    if drop_outside is not None and drop_outside != False:
        ...
        ground_track_points_4326 = gpd.GeoSeries(gpd.points_from_xy(ds.lon_20_ku, ds.lat_20_ku), crs=4326)
        try:
    ...
    ds = ds.assign_attrs(coherence_threshold=coherence_threshold,
...
def append_ambiguous_reference_elevation(ds, dem_file_name_or_path: str = None):
    ...
    with get_dem_reader((ds if dem_file_name_or_path is None else dem_file_name_or_path)) as dem_reader:
        ...
        ds = ds.assign(xph_x=(("time_20_ku", "ns_20_ku", "phase_wrap_factor"), x), 
...
def append_best_fit_phase_index(ds, best_column: callable = None) -> xr.Dataset:
    ...
    ds = ds.assign(ph_idx=(("time_20_ku", "ns_20_ku"),
    ...
    def find_group_ph_idx(elev_diff, group_ids):
        ...
        return out
    ds["ph_idx"] = xr.apply_ufunc(find_group_ph_idx, 
                                    ds.xph_elev_diffs, 
                                    ds.group_id,
                                    input_core_dims=[["ns_20_ku", "phase_wrap_factor"], ["ns_20_ku"]],
                                    output_core_dims=[["ns_20_ku"]])
    ds["ph_idx"] = xr.where(ds.group_id.isnull(), np.abs(ds.xph_elev_diffs).idxmin("phase_wrap_factor"), ds.ph_idx)
    return ds
...
def get_rgi_o2(ds) -> str:
    ...
    return rgi_o2_gpdf[rgi_o2_gpdf.contains(
            gpd.points_from_xy(ds.lon_20_ku, ds.lat_20_ku, crs=4326).unary_all(method="coverage").centroid
        )].long_code.values[0]
...
def get_phase_outlier(ds, tol: float|None = None):
    ...
    return np.abs(np.exp(1j*ds.ph_diff_waveform_20_ku) - ds.ph_diff_complex_smoothed) > tol
...
def locate_ambiguous_origin(ds):
    ...
    r_N = WGS84_ellpsoid.a/np.sqrt(1-WGS84_ellpsoid.es*np.sin(np.deg2rad(ds.lat_20_ku))**2)
    ...
    theta = np.arcsin(-(ds.ph_diff_waveform_20_ku + ds.phase_wrap_factor*2*np.pi)
    ...
    lons, lats = WGS84_ellpsoid.fwd(lons=ds.lon_20_ku.expand_dims({"ns_20_ku": ds.ns_20_ku.size,
                                                                "phase_wrap_factor": ds.phase_wrap_factor.size}, [-2, -1]),
                                lats=ds.lat_20_ku.expand_dims({"ns_20_ku": ds.ns_20_ku.size,
                                                                "phase_wrap_factor": ds.phase_wrap_factor.size}, [-2, -1]),
                                az=ds.azimuth.expand_dims({"ns_20_ku": ds.ns_20_ku.size,
                                                            "phase_wrap_factor": ds.phase_wrap_factor.size}, [-2, -1])+90,
                                dist=dist_off_groundtrack)[:2]
    return ds.assign(xph_lons=(("time_20_ku", "ns_20_ku", "phase_wrap_factor"), lons),
...
def ref_range(ds) -> xr.DataArray:
    ...
    """
    # make property?
    corrections = ds.mod_dry_tropo_cor_01 \
                    + ds.mod_wet_tropo_cor_01 \
                    + ds.iono_cor_gim_01 \
                    + ds.pole_tide_01 \
                    + ds.solid_earth_tide_01 \
                    + ds.load_tide_01
    return ds.window_del_20_ku/np.timedelta64(1, 's') / 2 * speed_of_light \
            + np.interp(ds.time_20_ku, ds.time_cor_01, corrections)
...
def tag_groups(ds) -> xr.Dataset:
    ...
    any_separator = np.logical_or(*xr.align(get_phase_jump(ds), gap_separator, join="outer"))
    ...
    group_tags = rising_edge_per_waveform_counter \
        + xr.DataArray(data=np.arange(len(ds.time_20_ku))*len(ds.ns_20_ku), dims="time_20_ku")
    group_tags = xr.align(group_tags, ds.power_waveform_20_ku, join="right")[0].where(~ignore_mask)
...
def to_l2(ds, out_vars: list|dict = None, *,
    ...
    if swath_or_poca == "swath":
        tmp = ds[out_vars+retain_vars].where(~ds.exclude_mask)\
        ...
        if len(waveforms_with_poca) == 0:
            return gpd.GeoDataFrame()
        tmp = ds[out_vars+retain_vars+["ph_idx"]].sel(time_20_ku=waveforms_with_poca)\
        ...
        swath = ds.to_l2(out_vars, retain_vars=retain_vars, swath_or_poca="swath", **kwargs)
        poca = ds.to_l2(out_vars, retain_vars=retain_vars, swath_or_poca="poca", **kwargs)
        return swath, poca
    else:
...
def unwrap_phase_diff(ds) -> xr.Dataset:
    ...
    if ds.attrs["smooth_phase_difference"]: #
        ds["ph_diff_waveform_20_ku"] = xr.where(ds.ph_diff_complex_smoothed.isnull(),
                                                    ds.ph_diff_waveform_20_ku,
                                                    xr.apply_ufunc(np.angle, ds.ph_diff_complex_smoothed))
        
    ds["ph_diff_waveform_20_ku"] = xr.apply_ufunc(unwrap, 
                                                    ds.ph_diff_waveform_20_ku, 
                                                    ds.group_id,
